...
int main(int argc, char *argv[])
{
	...
	int x4_id = stateVars.declareVar("x5");
	...
	Expression_AST<Real> deriv_x0("x3 - 0.25"); // theta_r = 0
	Expression_AST<Real> deriv_x1("x4 + 0.25");
	Expression_AST<Real> deriv_x2("x5");
	Expression_AST<Real> deriv_x3("9.81 * u0");
	Expression_AST<Real> deriv_x4("-9.81 * u1");
	Expression_AST<Real> deriv_x5("u2 - 9.81");
	...
	ode_rhs[u0_id] = deriv_u1;
	ode_rhs[u0_id] = deriv_u2;
	 ...
	 */
	double w = stod(argv[1]); // 0.0125
	int steps = stoi(argv[2]);
	Interval init_x0(0.0375 - w, 0.0375 + w), init_x1(0.0125 - w, 0.0125 + w), init_x2(0), init_x3(0), init_x4(0), init_x5(0);
	...
	string nn_name = "systems_with_networks/qmpc/quadrotor_mpc_tanh20x20";
	...
	ofstream result_output("./outputs/qmpc_tanh20x20_" + to_string(if_symbo) + ".txt");
	...
	plot_setting.plot_2D_octagon_MATLAB("qmpc_tanh20x20_x0_x3" + to_string(if_symbo), result);
